...
- [ ] The frontier is described as a **total function of issue state** — every
      open issue is exactly one of: unblocked, dependency-blocked,
      cross-milestone-blocked, parked, escalated — and §4 reports
      cross-milestone-blocked issues alongside escalated/parked.
- [ ] `skills/implement/SKILL.md`'s review gate loop and `skills/plan/SKILL.md`'s
      ...
      failure twice -> stop) **by reference** — no restated or copied rule.
...
- `skills/implement/SKILL.md`: the wave loop (§2 dispatch/await/recompute), the
  frontier definition (§1), the escalate/park report (§4), and the per-issue
  review gate (§3) — the loop-correctness loci only.
...
| The frontier is a **total function** of issue state (unblocked / dependency-blocked / cross-milestone-blocked / parked / escalated) | prior-art (task-master DAG frontier filters): membership is a queryable total set, not an emergent happy-path condition | 2026-07-08 |
...
- [ ] The §4 report enumerates cross-milestone-blocked issues alongside
      escalated (`needs:planning`) and parked (`blocked:human`).
- [ ] `skills/implement/SKILL.md` §3 review gate and `skills/plan/SKILL.md` step-6
      ...
      (no copied "same failure twice" restatement beyond the reference itself).
