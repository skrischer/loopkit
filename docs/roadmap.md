...
| structural-verify — set loopkit's `Verify` (currently `none`) to the native `claude plugin validate` (JSON validity + skill/agent/command frontmatter + duplicate names + path traversal — no new deps); wire it into the per-PR + milestone-QA gates; add a thin shell/git check only for loopkit invariants the native validator misses | [spec](specs/spec-structural-verify.md) | [#13](https://github.com/skrischer/loopkit/milestone/13) |
